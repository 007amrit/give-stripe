 ...
 * @since 2.5.0
 */
class ApplePayGateway extends CreditCardGateway
{
    ...
    public function __construct(SubscriptionModule $subscriptionModule = null)
    {
        ...
        $this->errorMessages['accountConfiguredNoSsl'] = esc_html__(
            'Apple Pay button is disabled because your site is not running securely over HTTPS.',
            'give-stripe'
        );
        $this->errorMessages['accountNotConfiguredNoSsl'] = esc_html__(
            'Apple Pay button is disabled because Stripe is not connected and your site is not running securely over HTTPS.',
            'give-stripe'
        );
        $this->errorMessages['accountNotConfigured'] = esc_html__(
            'Apple Pay button is disabled. Please connect and configure your Stripe account to accept donations.',
            'give-stripe'
        );
    }
    ...
    public function getLegacyFormFieldMarkup(int $formId, array $args): string
    {
        ...
        // Donor will see apply pay button instead of "Donate Now" button to process donation.
        return '';
    }
    ...
    {
        return esc_html__('Stripe - Apple Pay', 'give-stripe');
    }
    ...
    {
        return esc_html__('Apple Pay', 'give-stripe');
    }
}
